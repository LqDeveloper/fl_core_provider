...
#### 问题 ③：是否在 PageView 或 TabBarView 中？（必问，不要跳过）

这个问题必须问，因为它的答案会改变生成出来的页面代码本身，事后补不回来——切页生命周期不生效时**不会报错也不会警告**，只是 `onPageStart` / `onPageStop` 永远不来，很难在联调时定位。

- **否** → 普通路由页，无需额外配置。生成时不要加 `observePageView`。
- **是** → 需要**两侧同时配合**，缺一侧静默失效：
  1. **页面侧**（本次生成的文件）：override `bool get observePageView => true;`
     生成脚本传 `-p true`，Stateless / Stateful 都支持这个 getter。
  2. **宿主侧**（构建 `PageView` / `TabBarView` 的那个 widget）：用
     `FlPageViewScope.wrapChildren` / `wrapItemBuilder` 包裹子页。
     宿主通常是已有代码，不在本次生成范围内——**生成完要明确提醒用户去补这一步**，
     并给出对应的 `wrapChildren` 代码片段（见下节
     [PageView / TabBarView 使用指南](#pageview--tabbarview-使用指南)）。
  - 如需保持页面状态（滚动位置、表单输入），推荐 Stateful 并设置 `wantKeepAlive = true`。

> **用户答不上来时怎么办**：不要默认「否」就直接生成。先看宿主代码——grep 目标项目里的
> `PageView(`、`PageView.builder`、`TabBarView(`，看这个页面是否出现在某个 `children:` 列表里。
> 能确认再决定；确实无法确认时，按「否」生成并在结尾明确告知用户：
> 「如果这个页面会放进 PageView/TabBarView，还需要加 `observePageView => true` 并在宿主侧包
> `FlPageViewScope`」。
...
# Stateful + Events
bash fl_core_provider/scripts/generate.sh -n Login -t stateful -c events -e loadingChanged,loginSuccess

# Stateful + 非 keepAlive + 自定义输出目录
bash fl_core_provider/scripts/generate.sh -n Settings -t stateful -k false -d lib/screens

# 位于 PageView / TabBarView 中的子页：-p true 会写入 observePageView => true
bash fl_core_provider/scripts/generate.sh -n Home -t stateful -p true
```

> **注**：`-p true` 只解决**页面侧**的一半。宿主侧还必须用 `FlPageViewScope.wrapChildren` 包裹，
> 两者缺一即静默失效（见 [PageView / TabBarView 使用指南](#pageview--tabbarview-使用指南)）。
> `-k` 独立控制状态是否保留，与 `-p` 无关。
...
# Stateful, keepAlive=true, route name, custom output dir
./fl_core_provider/scripts/generate.sh -n Settings -t stateful -k true -r settings -d lib/screens
```
...
7. **Never silently skip 问题 ③（PageView/TabBarView）** — the answer changes the generated file. If the page lives in a `PageView` / `TabBarView`, emit `observePageView => true` **and** tell the user, in the closing summary, that the host still needs `FlPageViewScope.wrapChildren`. Half the pairing produces code that compiles, runs, and silently never fires page-switch lifecycle — the worst possible failure mode to hand someone.

## 6.5 PageView / TabBarView 使用指南

当页面是 `PageView` / `TabBarView` 的一页（含底部导航翻页、顶部 Tab 切换等）时，可以让
`onPageStart` / `onPageResume` 按"切到当前页"驱动（滑走时发 `onPagePause` / `onPageStop`），
而不是按路由变化驱动。`FlPageViewScope` 经 `fl_core_provider` re-export，
`import 'package:fl_core_provider/fl_core_provider.dart'` 即可直接用。

### ⚠️ 两侧必须配合：`FlPageViewScope` + `observePageView => true`

这是本节最容易出错的地方，先说清楚：**宿主包 Scope 和子页声明 `observePageView` 是两件独立的事，
必须同时做到，缺一侧完全无效。**

| 宿主包 `FlPageViewScope` | 子页 `observePageView => true` | 结果 |
| :---: | :---: | --- |
| ✅ | ✅ | 切页生命周期正常工作 |
| ✅ | ❌ | Scope 下发了页码，但子页压根不去读 —— 首帧即判定可见 |
| ❌ | ✅ | 子页想读，但向上找不到 Scope —— 回退成普通路由页 |
| ❌ | ❌ | 普通路由页 |

机制上说：`FlPageViewScope` 负责**往下游 context 发布页码**，`observePageView` 负责决定
**这个页面要不要去订阅**。发布者和订阅者是解耦的，所以任何一侧缺失都不会有编译错误、
不会有断言、也不会打日志——表现就是"生命周期回调不触发"，很容易被误判成框架 bug。

`observePageView` 在 `FlPageMixin` / `FlStateBuilder` 里的默认值是 **`false`**，
也就是说**默认不参与**切页判定，必须显式 override 才开启：

```dart
// Stateful
class _HomePageState extends FlBasePageState<HomePage, HomeController> {
  @override
  bool get observePageView => true;   // ← 必须写，默认 false

  @override
  HomeController createController(BuildContext context) => HomeController();
  // ...
}
...
class HomePage extends FlBasePage<HomeController> {
  ...
  @override
  bool get observePageView => true;   // ← 同名 getter，用法一致
...
反过来，如果某个子页不需要参与切页判定（比如它是个常驻的静态说明页），保持默认
`false`、不 override 即可——同一个 `PageView` 里可以混着放。

### 核心 API

| API | 作用 |
|---|---|
| `FlPageViewScope.wrapChildren(controller:, children:)` | 给 `PageView` / `TabBarView` 的固定子列表逐页注入页码作用域，下标即页码 |
| `FlPageViewScope.wrapItemBuilder(controller:, builder:)` | 给 `PageView.builder` 的 `itemBuilder` 包一层，适合动态/懒加载页面列表 |
| `controller` | 支持 `PageController`（`PageView`）或 `TabController`（`TabBarView`/`TabBar`） |

被 `FlPageViewScope` 包裹**且**声明了 `observePageView => true` 的子页，其 `FlPageLifecycleMixin` 状态：
- **`isInPageView`**：本页是否位于 PageView/TabBarView 中；
- **`onPageViewChanged(from, to)`**：切页回调，所有页都会收到；
- 可见性为"从最外层到本层每层都停在本页"才算可见，因此**嵌套场景**（PageView 里再套 TabBarView）外层滑走时内层当前子页同样收到 stop，滑回来一起收到 start。

### Pattern A: 底部导航（PageView + BottomNavigationBar）
    ...
    return Scaffold(
      body: PageView(
        ...
        children: FlPageViewScope.wrapChildren(
          controller: _controller,
          children: const [
            // 每个子页都是 FlBasePage / FlBasePageState，
            // 且都必须自己声明 observePageView => true，否则收不到切页事件
            HomePage(),
            SearchPage(),
            ProfilePage(),
          ],
...
### Pattern B: 顶部 Tab（TabBar + TabBarView）

`TabBarView` 与 `TabBar` 通过 `DefaultTabController` 或显式 `TabController` 共享同一个 `controller`，用 `DefaultTabController.of(context)` 取出即可：
              ...
              child: Builder(
                builder: (context) => TabBarView(
                  ...
                  children: FlPageViewScope.wrapChildren(
                    controller: DefaultTabController.of(context),
                    children: const [
                      // 同样：每个子页需自带 observePageView => true
                      HomePage(),
                      SearchPage(),
                      ProfilePage(),
                    ],
...
> **注意**：TabBarView 通过点击 Tab 切换时会缓存相邻子页；`PageView` 直接切则不缓存。若子页需要保留状态（如滚动位置、表单输入），子页用 Stateful（`FlBasePageState`) 并设 `wantKeepAlive = true`。

### Pattern C: 动态/懒加载页面列表（PageView.builder）
...
PageView.builder(
  ...
  itemBuilder: FlPageViewScope.wrapItemBuilder(
    controller: pageController,
    builder: (context, index) => PageItem(
      // PageItem 内部同样要声明 observePageView => true
...
### 排查清单：切页生命周期不触发时按顺序核对

`onPageStart` / `onPageStop` 没来的时候，99% 是下面两条之一，两条都要看：

1. **子页有没有 `@override bool get observePageView => true;`？**
   默认是 `false`，忘了写就完全不订阅页码。这是最常见的原因。
2. **宿主有没有用 `FlPageViewScope.wrapChildren` / `wrapItemBuilder` 包子页？**
   直接把 `children: const [HomePage(), ...]` 交给 `PageView` 是不够的。
   另外注意 `controller` 要传**同一个** `PageController` / `TabController` 实例，
   传成新建的实例等于没包。
3. 以上都对，再确认 `MaterialApp.navigatorObservers` 里挂了 `FlRouteObserver.instance`。

任一条不满足，子页都会被当成普通路由页：首帧即判定为可见，翻页不触发 start / stop，
且**不会有任何报错或警告**。

### 子页内感知切页

`FlBasePage` / `FlBasePageState` 的 controller（`FlBaseController`）可以覆写 `onPageViewChanged(from, to)` 感知页码变化——切页时**每个子页**都会收到，参数为 (旧页码, 新页码)：

```dart
class HomeController extends FlBaseController<HomeEvent> {
  @override
  void onPageViewChanged(int from, int to) {
    debugPrint('页码从 $from 切到 $to');
  }
}
```

> **说明**：`FlBasePage` / `FlBasePageState` 这一层**不暴露** `isInPageView` 等只读 getter——`FlStateBuilder` 只把 `onPageViewChanged` 与生命周期事件转发给 controller。如需这些只读状态，请改用 `FlPageLifecycleMixin`（见 `fl_state_lifecycle`）自己托管 State 生命周期，或借助 `FlPageViewScope.depthOf(context)` 判定嵌套深度。子页能感知的切页事件已足以驱动数据加载/暂停：`onPageStart` 在当前页被切到时发出，滑走后发 `onPageStop`。

若整个页面列表量级大（几百个实例），`FlPageViewScope` 每层只监听自己与父层控制器，通知量是 O(层数)，与页面数无关，可放心使用。
...
### Example 2: Login page (Stateful, keepAlive — used in PageView/TabBarView)

> 本页作为 `PageView` / `TabBarView` 的一页，所以页面侧声明了 `observePageView => true`，
> 并用 `wantKeepAlive => true` 保留表单输入。**宿主侧还必须用 `FlPageViewScope.wrapChildren`
> 包裹本页**，否则这里的 `observePageView` 没有 Scope 可读、等于没写。
> 宿主用法见 [PageView / TabBarView 使用指南](#pageview--tabbarview-使用指南)。
...
class _LoginPageState extends FlBasePageState<LoginPage, LoginController> {
  ...
  @override
  bool get observePageView => true;   // 本页在 PageView/TabBarView 中，默认 false 必须显式开启
